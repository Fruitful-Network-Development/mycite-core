...
from .geometry_datums import geometry_template_spec
...
def _build_default_required_refs(intent: dict[str, Any]) -> list[str]:
    out: list[str] = []
    for key in ("taxonomy_ref", "resource_ref", "parent_ref", "boundary_ref"):
...
def preview_write_intent(
    *,
    ...
    local_id = _as_text(fields.get("local_id") or intent.get("local_id"))
    if not local_id:
    ...
    if isinstance(external_writes, list):
        write_actions.extend([dict(item) for item in external_writes if isinstance(item, dict)])
    write_actions.append({"action": "create_target", "canonical_ref": local_id})

    config_updates: list[dict[str, Any]] = []
    if contract is not None:
        current = get_path(current_config, contract.target_path)
        config_updates.append({"path": contract.target_path, "previous": current, "next": local_id, "update_scope": contract.update_scope})

    return WritePreviewResult(
        ok=not errors,
        intent={"intent_type": intent_type, "template_id": template_id, "field_id": _as_text(intent.get("field_id")), "fields": dict(fields)},
        validation={
            "required_refs": required_refs,
    ...
    *,
    ...
    if not errors and preview.config_updates:
        ...
        for update in preview.config_updates:
            config_payload = set_path(config_payload, _as_text(update.get("path")), update.get("next"))
    ...
    return WriteApplyResult(
        ok=not errors,
        created_datum_refs=created_refs,
        updated_config_refs=config_updates,
        mutation_summary={"mutation_count": len(mutation_rows), "mutations": mutation_rows},
