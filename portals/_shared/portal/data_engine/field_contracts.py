...
def default_profile_field_contracts() -> dict[str, FieldContract]:
    return {
        ...
        "property_parcel_ref": FieldContract(
            field_id="property_parcel_ref",
            target_path="property.parcel_ref",
            datum_family="geometry.parcel",
            constraint_family="geometry.parcel_ref",
            write_modes=["select_existing_ref", "create_new_local_datum", "resolve_then_materialize"],
            multi_value=False,
            auto_create_prerequisites=True,
            update_scope="anthology+config_ref",
            lens_hint="geometry.parcel",
            allowed_template_ids=["geometry.parcel"],
            required_field_keys=["local_id", "title"],
            config_ref_surface=True,
        ),
        "property_plot_refs": FieldContract(
            field_id="property_plot_refs",
            target_path="property.plot_refs",
            datum_family="geometry.plot",
            constraint_family="geometry.plot_ref",
            write_modes=["select_existing_ref", "create_new_local_datum", "resolve_then_materialize"],
            multi_value=True,
            auto_create_prerequisites=True,
            update_scope="anthology+config_ref",
            lens_hint="geometry.plot",
            allowed_template_ids=["geometry.plot"],
            required_field_keys=["local_id", "title"],
            config_ref_surface=True,
        ),
        ...
        "inherited_product_profile_ref": FieldContract(
            field_id="inherited_product_profile_ref",
            target_path="agro.inherited.product_profile_ref",
            datum_family="agro.product_profile",
            constraint_family="samras.txa.inherited",
            write_modes=["stage_inherited_ref", "select_existing_ref"],
            multi_value=False,
            auto_create_prerequisites=False,
            update_scope="config_ref",
            lens_hint="agro.product_profile",
            allowed_template_ids=["agro.inherited.profile_8_5"],
            required_field_keys=["inherited_ref"],
            config_ref_surface=True,
        ),
        "inherited_supply_log_ref": FieldContract(
            field_id="inherited_supply_log_ref",
            target_path="agro.inherited.supply_log_ref",
            datum_family="agro.supply_log",
            constraint_family="samras.txa.inherited",
            write_modes=["stage_inherited_ref", "select_existing_ref"],
            multi_value=False,
            auto_create_prerequisites=False,
            update_scope="config_ref",
            lens_hint="agro.supply_log",
            allowed_template_ids=["agro.inherited.log_8_4"],
            required_field_keys=["inherited_ref"],
            config_ref_surface=True,
        ),
    }
