...
_LEGACY_ANTHOLOGY_FILENAME = "demo-anthology.json"
...
class JsonStorageBackend:
    ...
    def __init__(self, data_dir: Path | str):
        ...
        self._anthology_fallback_used = False
    ...
    def _read_table_path(self, table_id: str) -> Path:
        token = str(table_id or "").strip().lower()
        path = self._table_path(token)
        if token != "anthology":
            return path
        if path.exists() and path.is_file():
            self._anthology_fallback_used = False
            return path
        legacy = self.data_dir / _LEGACY_ANTHOLOGY_FILENAME
        if legacy.exists() and legacy.is_file():
            self._anthology_fallback_used = True
            return legacy
        self._anthology_fallback_used = False
        return path
    ...
    def read_payload(self, table_id: str) -> dict[str, Any]:
        path = self._read_table_path(table_id)
        if not path.exists() or not path.is_file():
            return {}
        try:
            payload = json.loads(path.read_text(encoding="utf-8"))
    ...
    def _anthology_rows(self, payload: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        if self._anthology_fallback_used:
            warnings.append("Deprecated anthology source in use: data/demo-anthology.json. Migrate to data/anthology.json.")
