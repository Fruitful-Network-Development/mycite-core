...
def _dispatch_newsletter(
    ...
    *,
    ...
    for recipient in recipients:
        ...
        try:
            sent = _aws_cli_json(["ses", "send-email", "--region", "us-east-1", "--output", "json"], input_payload=payload)
            result_row["message_id"] = _text((sent or {}).get("MessageId"))
    ...
    dispatch_row = {
        ...
        "reply_to_address": list_address,
        "subject": _text(subject),
        "body_text": _text(body_text),
        "target_count": len(recipients),
        "sent_count": sum(1 for row in results if _text(row.get("status")) == "sent"),
        "results": results,
    }
