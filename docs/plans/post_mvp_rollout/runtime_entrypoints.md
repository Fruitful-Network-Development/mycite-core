...
| `portal.home.tenant_status` | `instances._shared.runtime.tenant_portal_runtime.run_trusted_tenant_portal_home` | `band1.portal_home_tenant_status` | `Band 1 Trusted-Tenant Read-Only` | trusted-tenant read-only | serialized trusted-tenant home request payload, caller-supplied `data_dir`, `public_dir`, and `tenant_domain` | trusted-tenant runtime envelope with publication-backed tenant profile, visible Band 1 slice list, and read-only portal home shell composition |
| `portal.operational_status` | `instances._shared.runtime.tenant_operational_status_runtime.run_trusted_tenant_operational_status` | `band1.operational_status_surface` | `Band 1 Trusted-Tenant Read-Only` | trusted-tenant read-only | serialized operational-status request payload, caller-supplied audit storage file | trusted-tenant runtime envelope with explicit rollout/exposure posture, fixed recent-window (`20`) audit persistence summary, visible Band 1 slice list, and degraded-safe read-only shell composition |
| `portal.audit_activity` | `instances._shared.runtime.tenant_audit_activity_runtime.run_trusted_tenant_audit_activity` | `band1.audit_activity_visibility` | `Band 1 Trusted-Tenant Read-Only` | trusted-tenant read-only | serialized audit-activity request payload, caller-supplied trusted-tenant audit storage file | trusted-tenant runtime envelope with explicit rollout/exposure posture, the same fixed recent-window (`20`) local-audit seam used by operational status, bounded recent activity records, visible Band 1 slice list, and degraded-safe read-only shell composition |
| `portal.profile_basics_write` | `instances._shared.runtime.tenant_profile_basics_write_runtime.run_trusted_tenant_profile_basics_write` | `band2.profile_basics_write_surface` | `Band 2 Trusted-Tenant Writable Slice` | trusted-tenant writable | serialized profile-basics request payload, caller-supplied `data_dir`, `public_dir`, `tenant_domain`, and trusted-tenant audit storage file | trusted-tenant runtime envelope with bounded publication-backed profile basics editing, explicit read-after-write confirmation, local-audit receipt, recovery reference, and visible trusted-tenant slice list |
