...
| **E.1-E.3 — Canonical mutations (DONE 2026-05-13)** | Mutation runtime extended with `aws_csm_newsletter_contact_log`, `aws_csm_newsletter_profile`, `paypal_webhook` target authorities. Every FND-CSM mutation routes through `run_datum_workbench_mutation_action` and emits a NIMM envelope. `/__fnd/newsletter/{subscribe,unsubscribe,dispatch-result}` rewritten as ~5-line shims. | 2 hr | Medium | Yes |
| **F — Legacy route deprecation (DONE 2026-05-13)** | `/__fnd/newsletter/*` endpoints return `X-Deprecation: 1` + sunset date 2026-08-13 + successor route + reason headers. Routes stay live (unsubscribe links live in customer email archives). After 90 days of zero traffic, route bodies become `410 Gone`. | 30 min active + 90-day wait | Low | Yes |
