...
- **Legacy routes**: Redirects such as `/portal/peripherals` → `/portal/utilities`, `/portal/peripheral` → `/portal/utilities?tab=peripherals`, and `/portal/tools` → `/portal/utilities?tab=tools` remain for backward compatibility. `/portal/data` is canonical and `/portal/data/<path:tab_id>` now resolves to the canonical Data Tool entry.
- **Data workspace legacy shims**: TFF and FND now both run with `include_legacy_shims=False`; deprecated endpoints such as `/portal/api/data/tables` and `/portal/api/data/table/<id>/instances` are intentionally removed from canonical runtime behavior.
