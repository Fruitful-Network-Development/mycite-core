...
(function () {
  ...
  const WORKBENCH_RENDERERS = window.PortalShellWorkbenchRenderers || (window.PortalShellWorkbenchRenderers = {});
  const INSPECTOR_RENDERERS = window.PortalShellInspectorRenderers || (window.PortalShellInspectorRenderers = {});
  ...
  if (BODY_DATA) {
    BODY_DATA.setAttribute("data-shell-boot-state", "bundle_loaded");
  }
  ...
  function applyChrome(comp) {
    ...
    if (wb) {
      ...
      wb.setAttribute("data-foreground-visible", comp.foreground_shell_region === "center-workbench" ? "true" : "false");
    ...
    if (insp) {
      ...
      insp.setAttribute("data-primary-surface", inspectorRegion.primary_surface ? "true" : "false");
      insp.setAttribute("data-surface-layout", inspectorRegion.layout_mode || "sidebar");
    }
    ...
    if (kind === "tool_collapsed_inspector") {
      ...
      if (reopen) {
        reopen.addEventListener("click", function () {
          postShellChrome((wb.action_shell_chrome && typeof wb.action_shell_chrome === "object") ? wb.action_shell_chrome : { inspector_collapsed: false });
        });
      }
      return;
    }
    if (typeof WORKBENCH_RENDERERS[kind] === "function") {
      WORKBENCH_RENDERERS[kind]({
        region: wb,
        target: body,
        escapeHtml: escapeHtml,
        compactJson: compactJson,
        renderCtsGisSvg: renderCtsGisSvg,
        loadShell: loadShell,
  ...
  function renderInspector(region) {
    ...
    if (typeof INSPECTOR_RENDERERS[kind] === "function") {
      INSPECTOR_RENDERERS[kind]({
        region: region,
        titleEl: titleEl,
        target: content,
        escapeHtml: escapeHtml,
  ...
  function showFatal(message) {
    ...
    var controlPlaceholder = document.getElementById("v2-control-panel-placeholder");
    if (controlPlaceholder) {
      controlPlaceholder.textContent = message;
    }
    var body = document.getElementById("v2-workbench-body");
    if (body) {
