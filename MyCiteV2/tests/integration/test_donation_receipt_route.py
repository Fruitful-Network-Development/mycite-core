...
# NOTE: TestDonateHtmlReceiptInjection was removed here — it asserted on deployed
# artifacts under /srv/webapps/clients/** (owned by the webapps repo, generated by
# render_manifest.py), so it could never run hermetically in mycite-core CI (it
# skipUnless'd the absolute path and only "passed" on the deploy host). donate.js
# null-checks `#receipt-details` (the placeholder is now optional, so the old
