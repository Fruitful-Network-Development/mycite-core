...
class AnalyticsDataSourceTests(unittest.TestCase):
    ...
    def test_data_source_kind_pending_when_no_events(self) -> None:
        # Insights derive on-demand from the monthly analytics leaflet under
        # <webapps_root>/clients/_shared/site-core/analytics. The renderer
        # returns ``kind=pending`` + a notice when no leaflet exists yet for
        # the domain's entity. The 2nd positional arg is now ``private_dir``;
        # with no webapps_root the store anchors the analytics dir under it.
        out = _build_analytics_extension_payload("example.org", "/nonexistent")
        ds = out["data_source"]
        self.assertEqual(ds["kind"], "pending")
        self.assertEqual(
            ds["events_dir"], "/nonexistent/clients/_shared/site-core/analytics"
        )
