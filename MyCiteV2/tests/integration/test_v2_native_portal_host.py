...
class V2NativePortalHostTests(unittest.TestCase):
    ...
    def test_portal_and_health_are_native_v2_without_admin_bridge_route(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(bundle.get("canonical_shell_asset_path"), TRUSTED_TENANT_STATIC_BUNDLE_PATH)
            self.assertTrue(bundle.get("canonical_shell_asset_present"))
            self.assertTrue(bundle.get("v2_portal_shell_js_present"))
            support_assets = bundle.get("support_assets") or {}
            self.assertTrue(support_assets.get("v2_portal_shell_region_renderers_js_present"))
            self.assertTrue(support_assets.get("v2_portal_shell_core_js_present"))
            self.assertTrue(support_assets.get("v2_portal_workbench_renderers_js_present"))
            self.assertTrue(support_assets.get("v2_portal_inspector_renderers_js_present"))
            self.assertTrue(support_assets.get("v2_portal_shell_watchdog_js_present"))
    ...
    def test_portal_static_css_and_shell_markup(self) -> None:
        with TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.assertIn(b'class="ide-logo"', home.data)
                self.assertIn(b'data-shell-boot-state="template"', home.data)
                self.assertIn(b"v2_portal_shell.js", home.data)
                self.assertIn(b"portalInspectorTransientMount", home.data)
                self.assertNotIn(b"v2_portal_shell_core.js", home.data)
                self.assertNotIn(b"v2_portal_workbench_renderers.js", home.data)
                self.assertNotIn(b"v2_portal_inspector_renderers.js", home.data)
                self.assertNotIn(b"v2_portal_shell_watchdog.js", home.data)
            ...
            try:
                ...
                self.assertNotIn(b"v2_portal_shell_core.js", system.data)
