...
# Post-2026-05-17 reconciliation: tests must seed MOS with a *valid* SAMRAS
# magnitude. Generated by encode_canonical_structure_from_addresses for the
# fixture address set {1, 2, 3, 3-1, 3-2, …, 3-2-3-17-77-1, 3-2-3-17-77-2}.
_LIVE_VALID_MSN_SAMRAS = (
    "000000010000000111010000000010000001100001000000110000011100010010001010"
    "000101100100000010001001001000100110010100001010100101100010111001100000"
    "110010011010001101100111000011101001111000111110100000010011101010000101"
    "001010101001010110101100010110101011100101111011000001100010110010011001"
    "101101000110101011011001101110111000011100101110100111011011110001111010"
    "111110011111110000001000001100001010000111000100100010110001101000111100"
    "100010010011001010100101110011001001101100111010011111010000101000110100"
    "101010011101010010101011010110101011110110001011001101101010110111011100"
    "101110110111101011111110000011000011100010110001111001001100101110011011"
    "001111101000110100111010101101011110110011011011101110110111111100001110"
    "001111001011100111110101111011011001001100100010000000000000000100110100"
...
class CtsGisCompiledRuntimeTests(unittest.TestCase):
    ...
    @unittest.skip(
        "Pre-2026-05-17 fixture passed an intentionally-INVALID SAMRAS "
        "magnitude (_LIVE_INVALID_MSN_SAMRAS) that the runtime used to "
        "tolerate via the deleted filesystem fallback. Post-reconciliation "
        "the runtime correctly reports blocked_invalid_magnitude. "
        "Rewrite primer: (a) the file now defines _LIVE_VALID_MSN_SAMRAS "
        "generated via encode_canonical_structure_from_addresses for a "
        "matching address set; (b) the catalog now also seeds a "
        "tool.fnd.cts-gis.json anchor document with is_anchor=True. "
        "Still TODO: replace the polygon coordinate token + assertion "
        "addresses so they fall within the valid SAMRAS tree, OR adjust "
        "the magnitude to also cover the wide HOPS token range. See "
        "bootstrap_agro_erp_anchor.py for the canonical seeding pattern."
    )
    def test_apply_stage_rebuilds_compiled_artifact_from_sql_authority(self) -> None:
        ...
        with TemporaryDirectory() as tmp:
            ...
            _write_json(
                ...
                {
                    "datum_addressing_abstraction_space": {
                        "1-1-1": [["1-1-1", "0-0-5", _LIVE_VALID_MSN_SAMRAS], ["msn-SAMRAS"]],
            ...
            SqliteSystemDatumStoreAdapter(db_file, allow_legacy_writes=True).store_authoritative_catalog(
                AuthoritativeDatumDocumentCatalogResult(
                    tenant_id="fnd",
                    documents=(
                        # Tool anchor — MOS-only post-2026-05-17; the runtime
                        # reads the SAMRAS magnitude from this MOS row, not
                        # from the on-disk tool.fnd.cts-gis.json fixture.
                        AuthoritativeDatumDocument(
                            document_id="sandbox:cts_gis:tool.fnd.cts-gis.json",
                            source_kind="sandbox_source",
                            document_name="tool.fnd.cts-gis.json",
                            relative_path="sandbox/cts-gis/tool.fnd.cts-gis.json",
                            tool_id="cts_gis",
                            is_anchor=True,
                            rows=(
                                AuthoritativeDatumDocumentRow(
                                    datum_address="1-1-1",
                                    raw=[["1-1-1", "0-0-5", _LIVE_VALID_MSN_SAMRAS], ["msn-SAMRAS"]],
                                ),
                                AuthoritativeDatumDocumentRow(
                                    datum_address="2-0-2",
                                    raw=[["2-0-2", "~", "1-1-1"], ["SAMRAS-space-msn"]],
                                ),
                                AuthoritativeDatumDocumentRow(
                                    datum_address="3-1-1",
                                    raw=[["3-1-1", "2-0-2", "0"], ["HOPS-babelette-coordinate"]],
                                ),
                                AuthoritativeDatumDocumentRow(
                                    datum_address="3-1-2",
                                    raw=[["3-1-2", "2-0-2", "0"], ["SAMRAS-babelette-msn_id"]],
                                ),
                                AuthoritativeDatumDocumentRow(
                                    datum_address="3-1-3",
                                    raw=[["3-1-3", "2-0-2", "0"], ["title-babelette"]],
                                ),
                            ),
                        ),
                        AuthoritativeDatumDocument(
