...
class PortalRuntimePlatformContractTests(unittest.TestCase):
    def test_entrypoint_catalog_is_static_and_serializable(self) -> None:
        ...
        # FND-CSM surface removed — its entrypoints must NOT be in the catalog.
        self.assertNotIn("portal.system.tools.fnd_csm", [entry["entrypoint_id"] for entry in descriptors])
        self.assertNotIn("/portal/api/v2/system/tools/fnd-csm", [entry["route"] for entry in descriptors])
