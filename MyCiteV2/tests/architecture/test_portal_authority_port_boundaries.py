...
PACKAGE_DIR = PROJECT_ROOT / "packages" / "ports" / "portal_authority"
...
def _is_allowed_absolute_import(module_name: str) -> bool:
    ...
    if root_name in getattr(sys, "stdlib_module_names", set()):
        return True
    return module_name.startswith("MyCiteV2.packages.ports.portal_authority")


class PortalAuthorityPortBoundaryTests(unittest.TestCase):
