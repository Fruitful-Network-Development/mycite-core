...
PACKAGE_DIR = PROJECT_ROOT / "packages" / "ports" / "aws_narrow_write"
...
def _is_allowed_absolute_import(module_name: str) -> bool:
    ...
    if root_name in getattr(sys, "stdlib_module_names", set()):
        return True
    return module_name.startswith("MyCiteV2.packages.ports.aws_narrow_write")


class AwsNarrowWritePortBoundaryTests(unittest.TestCase):
    ...
    def test_source_contains_no_filesystem_or_broad_provider_ownership(self) -> None:
        violations: list[str] = []
