...
PACKAGE_DIR = PROJECT_ROOT / "packages" / "ports" / "audit_log"
...
def _is_allowed_absolute_import(module_name: str) -> bool:
    ...
    if root_name in getattr(sys, "stdlib_module_names", set()):
        return True
    return module_name.startswith("MyCiteV2.packages.core") or module_name.startswith("MyCiteV2.packages.ports.audit_log")


class AuditLogPortBoundaryTests(unittest.TestCase):
    def test_imports_remain_port_only_and_adapter_free(self) -> None:
        violations: list[str] = []
    ...
    def test_source_contains_no_runtime_filesystem_or_shell_ownership_leakage(self) -> None:
        violations: list[str] = []
