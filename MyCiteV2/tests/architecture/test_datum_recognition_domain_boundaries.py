...
PACKAGE_DIR = PROJECT_ROOT / "packages" / "modules" / "domains" / "datum_recognition"
...
def _is_allowed_absolute_import(module_name: str) -> bool:
    ...
    if root_name in getattr(sys, "stdlib_module_names", set()):
        return True
    return (
        module_name.startswith("MyCiteV2.packages.modules.domains.datum_recognition")
        or module_name.startswith("MyCiteV2.packages.ports.datum_store")
    )


class DatumRecognitionDomainBoundaryTests(unittest.TestCase):
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        ...
                        if module_name.startswith(FORBIDDEN_IMPORT_PREFIXES):
                            ...
                            violations.append(f"{path.name}: non-domain import {module_name}")
