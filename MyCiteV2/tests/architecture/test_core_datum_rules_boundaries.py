...
PACKAGE_DIR = PROJECT_ROOT / "packages" / "core" / "datum_rules"
...
def _is_allowed_absolute_import(module_name: str) -> bool:
    ...
    if root_name in getattr(sys, "stdlib_module_names", set()):
        return True
    return module_name.startswith("MyCiteV2.packages.core")


class CoreDatumRulesBoundaryTests(unittest.TestCase):
    def test_imports_remain_inward_and_standard_library_only(self) -> None:
        violations: list[str] = []
                ...
                if isinstance(node, ast.Import):
                    for alias in node.names:
                        ...
                        if module_name.startswith(FORBIDDEN_IMPORT_PREFIXES):
                            ...
                            violations.append(f"{path.name}: non-core import {module_name}")
    ...
    def test_source_contains_no_runtime_or_route_ownership_leakage(self) -> None:
        violations: list[str] = []
