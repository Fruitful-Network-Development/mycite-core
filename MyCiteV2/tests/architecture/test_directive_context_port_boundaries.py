...
PACKAGE_DIR = PROJECT_ROOT / "packages" / "ports" / "directive_context"
...
def _is_allowed_absolute_import(module_name: str) -> bool:
    ...
    if root_name in getattr(sys, "stdlib_module_names", set()):
        return True
    return module_name.startswith("MyCiteV2.packages.ports.directive_context")


class DirectiveContextPortBoundaryTests(unittest.TestCase):
    ...
    def test_source_contains_no_adapter_or_runtime_ownership_leakage(self) -> None:
        violations: list[str] = []
        for path in sorted(PACKAGE_DIR.glob("*.py")):
