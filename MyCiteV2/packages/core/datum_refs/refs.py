...
def _is_numeric_hyphen_token(token: str) -> bool:
    return bool(_NUMERIC_HYPHEN_TOKEN_RE.fullmatch(as_text(token)))


def _extract_datum_address_tail(token: str) -> str:
    parts = as_text(token).split("-")
...
def normalize_datum_ref(
    ...
    *,
    ...
    target_format = (as_text(write_format).lower() or "dot")

    msn_id = parsed.msn_id
    if not msn_id and (require_qualified or as_text(local_msn_id)):
