...
class AwsEc2RoleNewsletterCloudAdapter(AwsCsmNewsletterCloudPort):
    ...
    def get_or_create_secret_value(
        ...
        *,
        ...
        try:
            ...
            return as_text(response.get("SecretString"))
    ...
    def queue_dispatch_message(
        ...
        *,
        ...
        return as_text(response.get("MessageId"))
            ...
            out.append(
                {
                    "bucket": as_text(bucket),
                    "key": key,
                    "s3_uri": f"s3://{as_text(bucket)}/{key}",
                    "last_modified": _iso_timestamp(row.get("LastModified")),
                    "etag": as_text(row.get("ETag")),
                    "size": as_text(row.get("Size")),
                }
            )
    ...
    def caller_identity_summary(self) -> dict[str, Any]:
        try:
            ...
            return {
                "status": "ok",
                "account": as_text(response.get("Account")),
                "arn": as_text(response.get("Arn")),
                "user_id": as_text(response.get("UserId")),
            }
        except Exception as exc:  # noqa: BLE001
            return {"status": "error", "message": as_text(exc)}
    ...
    def queue_health_summary(self, *, queue_url: str, queue_arn: str, region: str) -> dict[str, Any]:
        try:
            ...
            return {
                "status": "ok",
                "queue_arn": as_text(values.get("QueueArn")) or queue_arn,
    ...
    def lambda_health_summary(self, *, function_name: str, region: str) -> dict[str, Any]:
        try:
            ...
            return {
                "status": as_text(configuration.get("State")).lower() or "ok",
                "function_arn": as_text(configuration.get("FunctionArn")),
                "role": as_text(configuration.get("Role")),
                "last_modified": as_text(configuration.get("LastModified")),
            }
        except Exception as exc:  # noqa: BLE001
            return {"status": "error", "message": as_text(exc)}
        ...
        *,
        ...
        for rule in rules:
            ...
            lambda_ok = any(
                (
                    ...
                    or as_text(action.get("FunctionArn")).endswith(expected_function_arn_suffix)
                )
            ...
            if matched_recipient:
                ...
                matching.append(
                    {
                        "rule_name": as_text(rule.get("Name")),
                        ...
                        "lambda_function_arn": as_text(primary_lambda.get("FunctionArn")),
                        "s3_bucket": as_text(primary_s3.get("BucketName")),
                        "s3_prefix": as_text(primary_s3.get("ObjectKeyPrefix")),
                        "s3_uri_prefix": (
                            f"s3://{as_text(primary_s3.get('BucketName'))}/{as_text(primary_s3.get('ObjectKeyPrefix'))}"
                            if as_text(primary_s3.get("BucketName"))
                            else ""
                        ),
                    }
                )
        if not matching:
            return {
                ...
                "expected_recipient": expected_email or as_text(expected_recipient),
