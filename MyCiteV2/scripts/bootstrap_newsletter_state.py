...
RUNTIME_SECRETS_SCHEMA = "mycite.service_tool.aws.newsletter_runtime_secrets.v2"
...
def _canonical_profile_payload(
    *,
    ...
    payload = dict(existing if _as_text(existing.get("schema")) == NEWSLETTER_PROFILE_SCHEMA else {})
    payload["schema"] = NEWSLETTER_PROFILE_SCHEMA
...
def run(*, private_dir: Path, tenant_id: str, prune_legacy_root: bool) -> dict[str, Any]:
    ...
    state = FilesystemNewsletterStateAdapter(private_dir)
    ...
    cloud = NewsletterCloudAdapter()
    service = NewsletterService(state, cloud, tenant_id=tenant_token)
