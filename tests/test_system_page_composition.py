...
class SystemPageCompositionTests(unittest.TestCase):
    ...
    def test_wiki_locks_unified_system_and_hidden_tool_home_framing(self) -> None:
        ...
        self.assertIn("tool mediation switches the foreground composition to control panel + interface panel only", (REPO_ROOT / "wiki/architecture/system-state-machine.md").read_text(encoding="utf-8"))
        self.assertIn("interface panel (right) becomes the primary visible mediation surface", (REPO_ROOT / "wiki/tools/tool-layer-mediation.md").read_text(encoding="utf-8"))
