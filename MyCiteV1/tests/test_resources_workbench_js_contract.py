...
class ResourcesWorkbenchJsContractTests(unittest.TestCase):
    def test_resources_loader_and_state_are_singular(self) -> None:
        ...
        self.assertIn('activeFileKey: "anthology"', js)
        self.assertIn("activateSystemWorkbench", js)
        self.assertNotIn("setDtWorkspaceTab", js)
        self.assertNotIn("WORKSPACE_TAB_STORAGE_KEY", js)
        self.assertNotIn("dtWorkspaceTabButtons", js)
        self.assertIn("dtSystemAitasStripEl", js)
        ...
        self.assertIn("systemMutate", js)
        self.assertIn("systemPublish", js)
        self.assertIn("js-system-create-row", js)
        self.assertIn("js-system-delete-row", js)
        self.assertIn("mycite:shell:file-focus-changed", js)
