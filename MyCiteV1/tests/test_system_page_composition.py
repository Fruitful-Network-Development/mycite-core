...
class SystemPageCompositionTests(unittest.TestCase):
    def test_data_tool_shell_center_only_no_embedded_inspector(self) -> None:
        partial = REPO_ROOT / "instances/_shared/runtime/flavors/fnd/portal/ui/templates/tools/partials/data_tool_shell.html"
    ...
    def test_system_shell_owns_inspector_slots(self) -> None:
        for flavor in ("fnd", "tff"):
            path = REPO_ROOT / f"instances/_shared/runtime/flavors/{flavor}/portal/ui/templates/services/system.html"
            text = path.read_text(encoding="utf-8")
            self.assertIn('id="systemShellInspectorRoot"', text)
            self.assertIn('data-interface-panel-root="system"', text)
            self.assertIn('id="systemToolInterfaceRoot"', text)
            self.assertIn('data-interface-panel-root="tool"', text)
            self.assertIn('data-tool-primary-host="true"', text)
            self.assertIn('aria-label="Tool interface mediation surface"', text)
            self.assertIn('aria-label="SYSTEM interface panel content"', text)
            self.assertIn('id="systemInspectorCardsMount"', text)
            self.assertIn('id="systemToolInterfaceBody"', text)
            self.assertIn('data-tool-panel-fill="true"', text)
    ...
    def test_system_resources_task_sidebar_and_workbench_query_links(self) -> None:
        for flavor in ("fnd", "tff"):
            ...
            self.assertIn('id="systemToolContextMount"', text)
    ...
    def test_base_template_exposes_inspector_block_and_transient_mount(self) -> None:
        base = REPO_ROOT / "instances/_shared/runtime/flavors/fnd/portal/ui/templates/base.html"
        ...
        self.assertIn('data-shell-composition="', text)
        self.assertIn('data-foreground-shell-region="', text)
        self.assertIn('data-foreground-visible="', text)
        self.assertIn('data-primary-surface="', text)
        self.assertIn('data-surface-layout="', text)
        self.assertIn('data-interface-panel-scroll-root="true"', text)
        self.assertIn('data-interface-panel-active-root="', text)
        self.assertIn('data-interface-panel-root="transient"', text)
    ...
    def test_menubar_css_locks_non_wrapping_system_shell_controls(self) -> None:
        css = (REPO_ROOT / "instances/_shared/runtime/flavors/fnd/portal/ui/static/portal.css").read_text(encoding="utf-8")
        js = (REPO_ROOT / "instances/_shared/runtime/flavors/fnd/portal/ui/static/portal.js").read_text(encoding="utf-8")
        ...
        self.assertIn(".ide-shell[data-shell-composition=\"tool\"] .ide-inspector__content {", css)
        self.assertIn("[data-interface-panel-root][hidden] {", css)
        self.assertIn(".ide-shell[data-shell-composition=\"tool\"] [data-interface-panel-root=\"transient\"] {", css)
        self.assertIn(".ide-shell[data-shell-composition=\"tool\"] .system-tool-interface__body {", css)
        self.assertIn(".tool-valueGrid {", css)
        self.assertIn("flex-wrap: nowrap;", css)
        self.assertIn(".ide-shell--system-workbench.ide-shell--workbench-tight .ide-menubar__pageSub", css)
        self.assertIn("activatePersistentRoot", js)
        self.assertIn("dismissTransient", js)
        self.assertIn('data-interface-panel-active-root', js)
        self.assertIn('inspector.setAttribute("data-surface-layout"', js)
    ...
    def test_wiki_locks_unified_system_and_hidden_tool_home_framing(self) -> None:
        wiki_readme = (REPO_ROOT / "docs/wiki/README.md").read_text(encoding="utf-8")
        shell_page = (REPO_ROOT / "docs/wiki/architecture/shell-and-page-composition.md").read_text(encoding="utf-8")
        provider_page = (REPO_ROOT / "docs/wiki/tools/provider-model.md").read_text(encoding="utf-8")
        agro_page = (REPO_ROOT / "docs/wiki/tools/agro-erp-mediation.md").read_text(encoding="utf-8")
        archive_page = (REPO_ROOT / "docs/wiki/archive/historical-lineage.md").read_text(encoding="utf-8")
        self.assertIn("primary maintained knowledge base", wiki_readme)
        self.assertIn("broader `docs/` tree remains active", wiki_readme)
        self.assertIn("no visible Local Resources or Inheritance tabs", shell_page)
        self.assertIn("`TOOL` mode", shell_page)
        self.assertIn("no foreground center workbench region", shell_page)
        self.assertIn("GET /portal/system", shell_page)
        self.assertIn("tool mediation switches the foreground composition to control panel + interface panel only", (REPO_ROOT / "docs/wiki/architecture/system-state-machine.md").read_text(encoding="utf-8"))
        self.assertIn("interface panel (right) becomes the primary visible mediation surface", (REPO_ROOT / "docs/wiki/tools/tool-layer-mediation.md").read_text(encoding="utf-8"))
    ...
    def test_inherited_inventory_api_exposes_grouped_by_source(self) -> None:
        ...
        data_workspace = REPO_ROOT / "instances/_shared/portal/api/data_workspace.py"
        ...
        runtime_body = (REPO_ROOT / "mycite_core/state_machine/view_model.py").read_text(encoding="utf-8")
        self.assertIn('"attention_address"', runtime_body)
        self.assertIn('"directive"', runtime_body)
        self.assertIn('"spatial"', runtime_body)
