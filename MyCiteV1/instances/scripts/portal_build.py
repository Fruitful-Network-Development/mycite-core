...
PORTALS_ROOT = INSTANCES_ROOT / "deployed"
...
SEED_PATTERNS = (
    ...
    "private/utilities/tools/fnd-ebi/*.json",
    "private/utilities/tools/newsletter-admin/newsletter-admin.*.json",
...
def _portal_dir(portal_id: str) -> Path:
    ...
    if not portal_dir.exists() or not portal_dir.is_dir():
        portal_dir = PORTALS_ROOT / _portal_instance_id_for(portal_id)
    if not portal_dir.exists() or not portal_dir.is_dir():
