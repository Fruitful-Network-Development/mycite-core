...
def _allowed_internal_roots() -> list[Path]:
    ...
    if raw:
        ...
        roots = [Path("/srv/webapps").resolve(), Path("/srv/webapps/clients").resolve()]
    out: list[Path] = []
    seen: set[Path] = set()
    for root in roots:
        if root in seen:
...
def derive_client_analytics_paths(site_root: str, *, now_utc: datetime | None = None) -> dict[str, Path]:
    ...
    canonical_events = analytics_root / "events" / f"{month_token}.ndjson"
    legacy_events = analytics_root / "evnts" / f"{month_token}.ndjson"
    return {
