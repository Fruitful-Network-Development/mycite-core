...
def _fallback_client_root(domain: str) -> Path:
    return Path("/srv/webapps") / _text(domain).lower()
    ...
    if root.exists() and root.is_dir():
        for path in sorted(root.glob("fnd-ebi.*.json")):
            ...
            if not domain or not site_root:
                continue
            out[domain] = Path(site_root)
    for domain in list(out.keys()):
        continue
    return out


def _configured_contact_log_path(private_dir: Path, domain: str) -> Path | None:
    payload = _read_json(newsletter_profile_path(private_dir, domain))
    raw_path = _text(payload.get("contact_log_path"))
    return Path(raw_path) if raw_path else None


def newsletter_contact_log_path(private_dir: Path, domain: str) -> Path:
    token = _text(domain).lower()
    configured_path = _configured_contact_log_path(private_dir, token)
    if configured_path is not None:
        return configured_path
    site_root = newsletter_site_roots(private_dir).get(token)
    if site_root is None:
        site_root = _fallback_client_root(token) / "frontend"
    client_root = site_root.parent
    return client_root / "contact" / f"{token}-contact_log.json"
